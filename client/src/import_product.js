  ...
  const importedProductsBarcodes = importedProducts.map((importedProduct) => {
    ...
    return (
      <div className='pt-2 d-flex justify-content-center' key={importedProduct.id}>
        <Barcode value={stringId.slice(-8)} text={importedProduct.name + ", " + importedProduct.sap_material_number + ", " + importedProduct.lot_number + ", " + importedProduct.shelf.name} lineColor='#00000' background='#FFFFFF' height={50} textAlign="center" fontSize={24} />
      </div>)
  })
